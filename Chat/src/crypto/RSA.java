...
public class RSA {
    ...
    public static void main(String[] args) throws Exception {
    	...
    	RSA rsa = new RSA();
        rsa.geraParChaves (new File ("chave.publica"+pid), new File ("chave.privada"+pid));

        //-- Cifrando a mensagem "Hello, world!"
        byte[] textoClaro = "Oi eu sou uma mensagem".getBytes("UTF-8");
        PublicKey pub = rsa.carregaChavePublica (new File ("chave.publica"+pid));
        byte[][] cifrado = rsa.cifra (pub, textoClaro);
        printHex (cifrado[0]);
        printHex (cifrado[1]);
        
        //-- Decifrando a mensagem 
        PrivateKey pvk = rsa.carregaChavePrivada (new File ("chave.privada"+pid));
        byte[] decifrado = rsa.decifra (pvk, cifrado[0], cifrado[1]);
        System.out.println (new String (textoClaro, "UTF-8"));        
        
        
        MessageDigest md = MessageDigest.getInstance("MD5");
        byte[] thedigest = md.digest(textoClaro);
        System.out.println(new String(thedigest));
        
        thedigest = md.digest(decifrado);
        System.out.println(new String(thedigest));
    }
}
